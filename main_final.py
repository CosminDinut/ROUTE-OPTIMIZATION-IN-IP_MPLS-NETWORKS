...
def run_genetic_algorithm():
    ...
    # Plot convergence graph (Fig. 4)
    gen = logbook.select("gen")
    min_fitness = logbook.select("min")
    plt.figure(figsize=(10, 6))
    plt.plot(gen, min_fitness, 'b-', marker='o', label='Best Fitness')
    plt.xlabel("GENERATION")
    plt.ylabel("BEST FITNESS SCORE")
    plt.title("Genetic Algorithm Convergence - Best Fitness per Generation")
    plt.grid(True)
    plt.annotate(f'{min_fitness[0]:.3f}', (gen[0], min_fitness[0]))
    plt.annotate(f'{min_fitness[10]:.3f}', (gen[10], min_fitness[10]))
    plt.annotate(f'{min_fitness[20]:.3f}', (gen[20], min_fitness[20]))
    plt.annotate(f'{min_fitness[30]:.3f}', (gen[30], min_fitness[30]))
    plt.annotate(f'{min_fitness[50]:.3f}', (gen[50], min_fitness[50]))
    plt.savefig("fig4_ga_convergence.png")
    plt.show()
...
def train_and_evaluate_rf(df):
    ...
    plt.title("Random Forest: Actual vs. Predicted Latency")
    plt.legend()
    plt.grid(True)
...
def plot_initial_data(df):
    """ Generates the initial traffic plot (similar to Fig. 3). """
    plt.figure(figsize=(15, 6))
    
    df_link1_normal = df[(df['LinkID'] == 1) & (df['Scenario'] == 'Normal')]
    df_link1_congested = df[(df['LinkID'] == 1) & (df['Scenario'] == 'Congested')]
    
    plt.plot(df_link1_normal['Timestamp'], df_link1_normal['Traffic (kbps)'], label='Normal_Traffic (kbps)')
    plt.plot(df_link1_congested['Timestamp'], df_link1_congested['Traffic (kbps)'], label='Congested_Traffic (kbps)', color='red')
    
    plt.title("Traffic (kbps) for Link-1 over 7 days (10-min intervals)")
    plt.xlabel("Time Interval")
    plt.ylabel("Traffic (kbps)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("fig3_traffic_simulation.png")
    plt.show()

def plot_optimization_results(df_before, df_after, title_prefix):
    """ Generates comparative plots for before and after optimization. """
    fig, axes = plt.subplots(1, 3, figsize=(20, 5))
    fig.suptitle(f"{title_prefix} Optimization Analysis", fontsize=16)

    metrics = ['Utilization (%)', 'Latency (ms)', 'Packet Loss (%)']
    
    for i, metric in enumerate(metrics):
        sns.kdeplot(df_before[metric], ax=axes[i], label='Before', fill=True)
        sns.kdeplot(df_after[metric], ax=axes[i], label='After', fill=True)
        axes[i].set_title(f'{title_prefix} - {metric}')
        axes[i].legend()
        axes[i].grid(True)
        
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(f"{title_prefix.lower().replace(' ', '_').replace('(', '').replace(')', '')}_optimization_analysis.png")
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_initial_data(network_df)

    # 3. Run optimizations and visualize results
    df_baseline_congested = network_df[network_df['Scenario'] == 'Congested']

    # Dynamic Weighted Dijkstra
    df_dijkstra_after = simulate_dynamic_dijkstra(df_baseline_congested)
    plot_optimization_results(df_baseline_congested, df_dijkstra_after, "Dynamic Weighted Dijkstra")

    # Linear Programming (LP)
    df_lp_after = simulate_lp_optimization(df_baseline_congested)
    plot_optimization_results(df_baseline_congested, df_lp_after, "Network Flow Optimization (LP)")

    # Reinforcement Learning (RL)
    df_rl_after = simulate_rl_optimization(df_baseline_congested)
    plot_optimization_results(df_baseline_congested, df_rl_after, "Reinforcement Learning")

    # Genetic Algorithm
    run_genetic_algorithm()
    
    # 4. Train and evaluate prediction model
    train_and_evaluate_rf(network_df)
    
    print("\nScript finished successfully. All plots and results have been generated.")