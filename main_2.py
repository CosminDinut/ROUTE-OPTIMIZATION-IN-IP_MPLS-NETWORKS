...
def simulate_dynamic_dijkstra(df_before):
    ...
    df_after['Utilization (%)'] *= 0.90  # 10% reduction
    df_after['Latency (ms)'] *= 0.80      # 20% reduction
    df_after['Packet Loss (%)'] *= 0.70  # 30% reduction
    return df_after
...
def simulate_lp_optimization(df_before):
    ...
    df_after['Latency (ms)'] *= 0.75      # 25% reduction
    df_after['Packet Loss (%)'] *= 0.60  # 40% reduction
    return df_after
...
def simulate_rl_optimization(df_before):
    ...
    df_after = df_before.copy()
    df_after['Utilization (%)'] *= 0.92  # 8% reduction
    df_after['Latency (ms)'] *= 0.85      # 15% reduction
    df_after['Packet Loss (%)'] *= 0.80  # 20% reduction
